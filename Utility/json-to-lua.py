...
# Paste the entire Override.lua contents here as a triple-quoted string
override_code = """
Food = {
...
def process_json(json_path, base_folder, output_base):
    ...
    waypoints = [vec_to_lua(vec) for vec in data['Bones']]
    # Determine which whitelist to use: CombatWhiteList or GatherWhiteList
    if 'CombatWhiteList' in data and data['CombatWhiteList'].strip():
        whitelist = data['CombatWhiteList']
    elif 'GatherWhiteList' in data and data['GatherWhiteList'].strip():
        whitelist = data['GatherWhiteList']
    else:
        whitelist = ""

    target_names = [f'"{name.strip()}"' for name in whitelist.split(',') if name.strip()]
    waypoints_str = ',\n    '.join(waypoints)
    target_names_str = ', '.join(target_names)

    # BlackListedAreas
    blacklist_lines = ""
    for area in data.get("BlackListedAreas", []):
        if isinstance(area, list) and len(area) == 3:
            blacklist_lines += f"BlackListPoint({area[0]}, {area[1]}, {area[2]})\n"

    # Top variables
    player_line = "Player = GetPlayer()\n"
    repair_line = f"UseDBToRepair({str(data.get('RepairAtVendor', False)).lower()})\n"
    repair_at_line = f"SetQuestRepairAt({data.get('RepairAt', 0)})\n"
    sell_line = f"UseDBToSell({str(data.get('SellGrays', False)).lower()})\n"
    sell_at_line = f"SetQuestSellAt({data.get('SellGraysAt', 0)})\n"
    ...
    lua = (
        ...
        "}\n\n"
        "SetFoodAndDrink()\n"
        f"GrindAndGatherPath(TableToStringList(TargetNames), TableToVecList(Waypoints), {combat_radius}, {autoskin}, \"Override\")\n"
        "SetVendor()\n"
        "SellAtVendor()\n"
    )
