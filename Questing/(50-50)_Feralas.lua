StartMobAvoidance();
UseDBToRepair(true);
UseDBToSell(true);
SetQuestRepairAt(30);
SetQuestSellAt(2);
IgnoreLowLevelQuests(false);

TurnInQuestUsingDB(3062);
TurnInQuestUsingDB(3063);
AcceptQuestUsingDB(7738); -- accept Perfect Yeti Hide
TurnInQuestUsingDB(7734);
TurnInQuestUsingDB(7738);
TurnInQuestUsingDB(3127);
AcceptQuestUsingDB(3129); -- accept Weapons of Spirit
TurnInQuestUsingDB(3129);
AcceptQuestUsingDB(1173); -- accept Challenge Overlord Mok'Morokk
CompleteObjectiveOfQuest(1173,1);
TurnInQuestUsingDB(1173);
AcceptQuestUsingDB(4502); -- accept Volcanic Activity
TurnInQuestUsingDB(2874);
TurnInQuestUsingDB(580);
TurnInQuestUsingDB(1122);
AcceptQuestUsingDB(7029); -- accept Vyletongue Corruption
AcceptQuestUsingDB(7064); -- accept Corruption of Earth and Seed
AcceptQuestUsingDB(7028); -- accept Twisted Evils
AcceptQuestUsingDB(7067); -- accept The Pariah's Instructions
AcceptQuestUsingDB(7044); -- accept Legend of Maraudon
CompleteObjectiveOfQuest(7029,2);
TurnInQuestUsingDB(7029);
TurnInQuestUsingDB(7064);
TurnInQuestUsingDB(7067);
TurnInQuestUsingDB(7028);
StopQuestProfile();