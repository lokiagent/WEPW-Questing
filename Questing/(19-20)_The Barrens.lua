StartMobAvoidance();
UseDBToRepair(true);
UseDBToSell(true);
SetQuestRepairAt(30);
SetQuestSellAt(2);
IgnoreLowLevelQuests(false);

TurnInQuestUsingDB(3261);
AcceptQuestUsingDB(882); -- accept Ishamuhale
CompleteObjectiveOfQuest(821,3);
CompleteObjectiveOfQuest(899,1);
CompleteObjectiveOfQuest(878,1);
CompleteObjectiveOfQuest(878,2);
CompleteObjectiveOfQuest(878,3);
AcceptQuestUsingDB(883); -- accept Lakota'mani
CompleteObjectiveOfQuest(880,1);
CompleteObjectiveOfQuest(882,1);
TurnInQuestUsingDB(888);
TurnInQuestUsingDB(1094);
AcceptQuestUsingDB(1095); -- accept Further Instructions
TurnInQuestUsingDB(865);
TurnInQuestUsingDB(1069);
TurnInQuestUsingDB(821);
AcceptQuestUsingDB(822); -- accept Chen's Empty Keg
TurnInQuestUsingDB(875);
AcceptQuestUsingDB(876); -- accept Serena Bloodfeather
TurnInQuestUsingDB(899);
TurnInQuestUsingDB(880);
AcceptQuestUsingDB(1489); -- accept Hamuul Runetotem
AcceptQuestUsingDB(3301); -- accept Mura Runetotem
AcceptQuestUsingDB(853); -- accept Apothecary Zamah
TurnInQuestUsingDB(878);
AcceptQuestUsingDB(5052); -- accept Blood Shards of Agamaggan
TurnInQuestUsingDB(5052);
TurnInQuestUsingDB(882);
AcceptQuestUsingDB(907); -- accept Enraged Thunder Lizards
TurnInQuestUsingDB(883);
CompleteObjectiveOfQuest(907,1);
TurnInQuestUsingDB(907);
AcceptQuestUsingDB(913); -- accept Cry of the Thunderhawk
CompleteObjectiveOfQuest(913,1);
TurnInQuestUsingDB(913);
AcceptQuestUsingDB(874); -- accept Mahren Skyseer
TurnInQuestUsingDB(853);
AcceptQuestUsingDB(962); -- accept Serpentbloom
AcceptQuestUsingDB(264); -- accept Until Death Do Us Part
TurnInQuestUsingDB(5724);
TurnInQuestUsingDB(5723);
TurnInQuestUsingDB(1489);
AcceptQuestUsingDB(1490); -- accept Nara Wildmane
TurnInQuestUsingDB(1490);
AcceptQuestUsingDB(914); -- accept Leaders of the Fang
TurnInQuestUsingDB(1507);
TurnInQuestUsingDB(1508);
TurnInQuestUsingDB(1509);
CompleteObjectiveOfQuest(852,1);
TurnInQuestUsingDB(852);
CompleteObjectiveOfQuest(876,1);
TurnInQuestUsingDB(1062);
AcceptQuestUsingDB(1063); -- accept The Elder Crone
AcceptQuestUsingDB(1068); -- accept Shredding Machines
TurnInQuestUsingDB(6523);
AcceptQuestUsingDB(6401); -- accept Kaya's Alive
TurnInQuestUsingDB(6629);
StopQuestProfile();